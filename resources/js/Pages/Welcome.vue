...
const isAdmin = computed(
  () => !!authUser.value?.is_admin || (authUser.value?.roles ?? []).includes("admin")
);
    ...
    >
      <div class="relative w-full max-w-2xl px-6 lg:max-w-7xl">
        <header class="grid grid-cols-2 items-center gap-2 py-10 lg:grid-cols-3">
          ...
          <nav v-if="canLogin" class="-mx-3 flex flex-1 items-center justify-end gap-2">
            <!-- Sudah login -->
            <template v-if="authUser">
              ...
              <!-- Dropdown User: Profile + Logout (untuk semua yang login) -->
